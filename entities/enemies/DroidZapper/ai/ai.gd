extends Node
class_name DroidZapperAI

enum State { IDLE, WAKE, RUN, ATTACK, KNOCKBACK, DEAD }
var state: State = State.IDLE

var player_in_aggro: bool = false
var target: Node2D = null

var knockback_timer: float = 0.0
var knockback_duration: float = 0.15
var knockback_force: float = 200.0

@onready var body: CharacterBody2D = get_parent()
@onready var pivot: Node2D = body.get_node("Marker2D")

@onready var anim: DroidZapperAnim = body.get_node("Anim")
@onready var stats: DroidZapperStats = body.get_node("Stats")

@onready var col_standard: CollisionShape2D = body.get_node("CollisionStandard")
@onready var attack_area: Area2D = pivot.get_node("AttackArea")
@onready var col_attack: CollisionShape2D = attack_area.get_node("CollisionAttack")

@onready var agro_area: Area2D = pivot.get_node("AgroArea")

@onready var entropy_system: EntropySystem = get_node("/root/entropy_system")

@export var attack_range: float = 40.0


func _ready() -> void:
	col_attack.disabled = true
	stats.apply_entropy(entropy_system.multiplier)

	# FIX: connect ke nama fungsi yg benar
	agro_area.body_entered.connect(_on_aggro_entered)
	agro_area.body_exited.connect(_on_aggro_exited)


func _physics_process(delta: float) -> void:
	if state == State.DEAD:
		return

	match state:
		State.IDLE: _process_idle()
		State.WAKE: _process_wake()
		State.RUN: _process_run(delta)
		State.ATTACK: _process_attack()
		State.KNOCKBACK: _process_knockback(delta)

func _process_idle() -> void:
	if player_in_aggro:
		state = State.WAKE
		anim.play_wake()


func _process_wake() -> void:
	if anim.is_finished():
		state = State.RUN
		anim.play_run()


func _process_run(delta: float) -> void:
	if target == null:
		return

	var dx: float = target.global_position.x - pivot.global_position.x
	var dir: float = sign(dx)

	_update_facing(dir)

	body.velocity.x = dir * stats.speed
	body.move_and_slide()

	anim.play_run()

	if pivot.global_position.distance_to(target.global_position) <= attack_range:
		state = State.ATTACK
		anim.play_attack()


func _process_attack() -> void:
	body.velocity.x = 0.0
	body.move_and_slide()

	col_attack.disabled = false
	_check_attack_hit()

	if anim.is_finished():
		col_attack.disabled = true
		state = State.RUN
		anim.play_run()


func _process_knockback(delta: float) -> void:
	body.move_and_slide()

	knockback_timer -= delta
	if knockback_timer <= 0.0:
		state = State.RUN
		anim.play_run()

func apply_knockback(source_pos: Vector2) -> void:
	if state == State.DEAD:
		return

	state = State.KNOCKBACK
	knockback_timer = knockback_duration

	var dir: float = sign(pivot.global_position.x - source_pos.x)
	if dir == 0.0:
		dir = 1.0

	body.velocity.x = dir * knockback_force
	anim.play_run()


func apply_damage(amount: float, source_pos: Vector2) -> void:
	var dmg: float = max(amount - stats.defense * 0.1, 1.0)
	stats.apply_damage(dmg)

	if stats.current_hp <= 0.0:
		_die()
	else:
		apply_knockback(source_pos)


func _die() -> void:
	state = State.DEAD
	body.velocity = Vector2.ZERO

	col_standard.disabled = true
	col_attack.disabled = true

	anim.play_death()

func _update_facing(dir: float) -> void:
	if dir == 0.0:
		return

	pivot.scale.x = -1.0 if dir < 0.0 else 1.0

	agro_area.position.x = abs(agro_area.position.x) * pivot.scale.x
	attack_area.position.x = abs(attack_area.position.x) * pivot.scale.x

func _check_attack_hit() -> void:
	if target == null:
		return

	var rect := col_attack.shape as RectangleShape2D
	var xform := col_attack.get_global_transform()

	var size: Vector2 = rect.size
	var area := Rect2(xform.origin - size * 0.5, size)

	if area.has_point(target.global_position):
		if target.has_method("apply_damage"):
			target.apply_damage(stats.attack, pivot.global_position)

func _find_player_root(node: Node) -> Node:
	while node != null and not node.is_in_group("player"):
		node = node.get_parent()
	return node


func _on_aggro_entered(body_node: Node) -> void:
	var obj = _find_player_root(body_node)
	if obj != null:
		player_in_aggro = true
		target = obj as Node2D


func _on_aggro_exited(body_node: Node) -> void:
	var obj = _find_player_root(body_node)
	if obj != null:
		player_in_aggro = false
		target = null
